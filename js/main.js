document.addEventListener("DOMContentLoaded", () => {
  ...
  portfolioLinks.forEach((link) => {
    link.addEventListener("click", (e) => {
      ...
      modalBody.innerHTML = `
        <h2>${
          link.closest(".portfolio-overlay").querySelector("h3").textContent
        }</h2>
        <p>${
          link.closest(".portfolio-overlay").querySelector("p").textContent
        }</p>
  ...
  const animateOnScroll = () => {
    const elements = document.querySelectorAll(
      ".section, .skill-card, .timeline-item, .portfolio-card"
    );
